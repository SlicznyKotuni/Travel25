...
const MapHandler = (() => {
    ...
    const init = () => {
        try {
            ...
            if (!mapElement) {
                console.warn('Element mapy nie istnieje');
                return;
            }
            ...
            document.addEventListener('tripLoaded', (e) => {
                if (e.detail && e.detail.data && e.detail.data.sections) {
                    updateMap(e.detail.data.sections);
                }
            });
        } catch (error) {
    ...
    const updateMap = (sections) => {
        try {
            ...
            if (!map || !markersLayer) {
                console.warn('Mapa nie jest zainicjalizowana');
                return;
            }
            ...
            console.error('Błąd aktualizacji mapy:', error);
        }
    };
    ...
    const centerMap = (location) => {
        try {
            ...
            console.error('Błąd centrowania mapy:', error);
        }
    };
